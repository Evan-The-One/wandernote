...
| 海报页数/点数/生成任务 | blocked | Bearer、报价和所有权协议已实现；Production 私有 Blob 尚未配置，且任务创建仍是长请求而非完全异步 worker |
| 私有海报短期下载 | implemented_waiting_real_device | Sharp 输出 JPEG、Private Blob 与5分钟 signed URL 已实现；等待存储配置和真机 |
