...
async function renew(){const refreshToken=getRefreshToken();if(refreshToken){try{const value=await requestPublic<{sessionToken:string;refreshToken:string}>("/api/miniapp/auth/refresh",{method:"POST",data:{refreshToken}});saveSession(value);return}catch{clearSession()}}await ensureWechatLogin()}
export async function request<T>(path:string,options:Partial<Taro.request.Option>={},retried=false){let token=getAccessToken();if(!token){await ensureWechatLogin();token=getAccessToken()}const response=await Taro.request({...options,url:`${origin}${path}`,header:{"content-type":"application/json",authorization:`Bearer ${token}`,...options.header}});if(response.statusCode===401&&!retried){await renew();return request<T>(path,options,true)}if(response.statusCode===401){clearSession();throw new Error("登录状态已过期，请重新进入")};if(response.statusCode>=400)throw new Error((response.data as {error?:{message?:string}})?.error?.message||"请求失败，请稍后重试");return response.data as T}
