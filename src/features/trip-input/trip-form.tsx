...
export function TripForm() {
  ...
  return <form onSubmit={submit} className="space-y-6">
    <section className="card rounded-[2rem] p-5 sm:p-8">
      <label className={`block ${coreQuestionClass}`}>你想去哪里？ <span className="text-[#c55e3d]">*</span><input required value={destination} onChange={(event) => setDestination(event.target.value)} className={`${inputClass} font-normal tracking-normal`} placeholder="例如：杭州" /></label>
