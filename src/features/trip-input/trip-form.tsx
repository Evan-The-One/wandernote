...
export function TripForm() {
  ...
  useEffect(()=>{const clear=()=>setClearConfirm(true);const restore=()=>window.location.reload();const recommend=()=>{setInspirationsExpanded(true);setTimeout(()=>window.dispatchEvent(new CustomEvent("trip-open-recommender")),50)};window.addEventListener("trip-request-clear",clear);window.addEventListener("trip-restore-draft",restore);window.addEventListener("trip-open-recommender",recommend,{once:true});const pending=sessionStorage.getItem("yijianchufa:pending-home-action");if(pending){sessionStorage.removeItem("yijianchufa:pending-home-action");setTimeout(()=>window.dispatchEvent(new CustomEvent(pending)),80)}return()=>{window.removeEventListener("trip-request-clear",clear);window.removeEventListener("trip-restore-draft",restore);window.removeEventListener("trip-open-recommender",recommend)}},[]);
  ...
  return <form onSubmit={submit} className="space-y-6">
    ...
    <div className="flex justify-end"><button type="button" onClick={()=>setClearConfirm(true)} className="rounded-full border border-black/10 px-4 py-2 text-sm font-semibold text-[#65706a]">一键清空选择</button></div>
