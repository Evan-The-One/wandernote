...
export function GeneratedPlanView({ tripId }: { tripId: string }) {
  ...
  return <>{error && <div className="page-shell mt-4 rounded-2xl bg-[#fff7e9] p-4 text-sm text-[#785f41]">{error}</div>}<TripPlanView plan={data.plan} input={data.input} onReviseDay={data.canEdit ? setRevisionDay : undefined} notice={notice} canUndo={data.canEdit && data.canUndo} onUndo={undoing ? undefined : undo} canEdit={data.canEdit} tripId={tripId} tripVersion={data.version} highlightedActivityIds={highlightedActivityIds} /><BetaFeedback tripId={tripId} /><div className="page-shell -mt-10 mb-20 text-center"><Link href="/#plan" className="inline-flex rounded-full border border-[#245b46]/20 bg-white px-5 py-2.5 text-sm font-bold text-[#245b46]">换个玩法重新安排</Link></div>{revisionDay && <DayRevisionPanel tripId={tripId} day={revisionDay} version={data.version} onClose={() => setRevisionDay(null)} onSuccess={(plan, version, summary,modifiedIds) => { setData({ ...data, plan, version, canUndo: true }); setNotice(summary); setHighlightedActivityIds(modifiedIds); setRevisionDay(null); localStorage.setItem(`wandernote:trip:${tripId}`, JSON.stringify({ input: data.input, plan, version })); window.setTimeout(()=>document.getElementById(`activity-${revisionDay.dayNumber}-${modifiedIds[0]}`)?.scrollIntoView({behavior:"smooth",block:"center"}),100); window.setTimeout(()=>setHighlightedActivityIds([]),3500); }} />}</>;
}
