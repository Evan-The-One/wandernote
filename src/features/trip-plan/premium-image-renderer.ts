...
function brand(ctx: CanvasRenderingContext2D, y: number, dark = true) { ctx.textAlign = "center"; ctx.fillStyle = dark ? "#204f3c" : "#ffffff"; ctx.font = `700 34px ${FONT}`; ctx.fillText("一键出发", WIDTH / 2, y); ctx.fillStyle = dark ? "#b27b32" : "#f1c777"; ctx.font = `500 14px ${FONT}`; ctx.fillText("O N E - C L I C K", WIDTH / 2, y + 30); ctx.textAlign = "left"; }
...
function drawPosterAdvice(ctx:CanvasRenderingContext2D,values:string[]){const layout=posterAdviceLayout();rounded(ctx,layout.x,layout.y,layout.width,layout.height,18,"#f4f3eb");ctx.fillStyle="#245b46";ctx.font=`800 19px ${FONT}`;ctx.fillText("出发前看一眼",layout.x+18,layout.y+27);ctx.font=`600 15px ${FONT}`;const cells=values.slice(0,6).map(tip=>wrap(ctx,tip,layout.cellWidth-52).slice(0,2));const rowHeights=[0,1].map(row=>Math.max(46,...cells.slice(row*3,row*3+3).map(lines=>lines.length*18+12)));const rowStarts=[layout.y+48,layout.y+48+rowHeights[0]!];cells.forEach((textLines,i)=>{const col=i%3,row=Math.floor(i/3),x=layout.x+18+col*layout.cellWidth,y=rowStarts[row]!;drawAdviceIcon(ctx,adviceIcons[i]!,x,y+4);ctx.fillStyle="#283c32";ctx.font=`600 15px ${FONT}`;textLines.forEach((line,lineIndex)=>ctx.fillText(line,x+32,y+16+lineIndex*18));});const contentEnd=rowStarts[1]!+rowHeights[1]!;if(contentEnd>layout.y+layout.height-27)throw Object.assign(new Error("海报建议区域文字溢出"),{code:"POSTER_CONTENT_OVERFLOW"});ctx.fillStyle="#5f6e65";ctx.font=`500 12px ${FONT}`;ctx.fillText("景点图片为 AI 视觉示意，请以实际现场为准。",layout.x+18,layout.y+layout.height-10);}
...
async function renderTimelinePosterV2(spec:Extract<TravelPosterSpec,{width:1024}>){
  return Promise.all(spec.pages.map(async(page,pageIndex)=>{const assets=page.days.flatMap(day=>day.activities.map(activity=>activity.visualAsset));const decoded=await Promise.all(assets.map(async asset=>{if(!asset.dataUrl.startsWith("data:image/"))throw Object.assign(new Error("活动图片缺失"),{code:"POSTER_VISUAL_MISSING"});const image=await loadImage(asset.dataUrl);if(image.naturalWidth<120||image.naturalHeight<80)throw Object.assign(new Error("活动图片不可用"),{code:"POSTER_GENERIC_ASSET_INVALID"});return [asset.id,image] as const;}));const images=new Map(decoded);
    const canvas=document.createElement("canvas");canvas.width=2048;canvas.height=3072;const ctx=canvas.getContext("2d");if(!ctx)throw new Error("当前浏览器无法生成海报");ctx.scale(2,2);ctx.fillStyle="#fbf8ef";ctx.fillRect(0,0,1024,1536);
    ...
    for(const [column,day] of page.days.entries()){const x=left+column*(cardWidth+gap);ctx.strokeStyle="#cad4ca";ctx.lineWidth=1.5;ctx.beginPath();ctx.roundRect(x,top,cardWidth,cardHeight,18);ctx.stroke();rounded(ctx,x+8,top+8,cardWidth-16,66,14,"#245b46");ctx.fillStyle="#fff";ctx.font=`800 28px ${FONT}`;ctx.fillText(`Day ${day.dayNumber}${day.date?`  ${day.date}`:""}`,x+28,top+40);ctx.fillStyle="#e6efe8";ctx.font=`600 16px ${FONT}`;ctx.fillText(day.title.slice(0,24),x+28,top+61);
      ...
      for(let i=0;i<count;i++){const activity=day.activities[i]!,rowY=top+88+i*rowHeight;if(i<count-1){ctx.strokeStyle="#cbd6cc";ctx.lineWidth=2;ctx.setLineDash([3,7]);ctx.beginPath();ctx.moveTo(timelineX,rowY+38);ctx.lineTo(timelineX,rowY+rowHeight+8);ctx.stroke();ctx.setLineDash([]);}ctx.fillStyle="#245b46";ctx.beginPath();ctx.arc(timelineX,rowY+25,6,0,Math.PI*2);ctx.fill();ctx.fillStyle="#245b46";ctx.font=`800 ${columns===2?18:20}px ${FONT}`;ctx.fillText(activity.time,textX,rowY+19);ctx.fillStyle="#15271e";ctx.font=`800 ${columns===2?22:24}px ${FONT}`;const nameLines=wrap(ctx,activity.name,textW).slice(0,2);nameLines.forEach((line,n)=>ctx.fillText(line,textX,rowY+48+n*25));ctx.fillStyle="#34443b";ctx.font=`500 ${columns===2?16:18}px ${FONT}`;const noteY=rowY+54+nameLines.length*24;wrap(ctx,activity.note,textW).slice(0,2).forEach((line,n)=>ctx.fillText(line,textX,noteY+n*20));const image=images.get(activity.visualAsset.id);if(!image)throw Object.assign(new Error("活动图片缺失"),{code:"POSTER_VISUAL_MISSING"});cropImage(ctx,image,imageX,rowY+9,imageW,imageH,12);}
    }
    ...
    ctx.textAlign="center";ctx.fillStyle="#245b46";ctx.font=`800 20px ${FONT}`;ctx.fillText("一键出发",512,1490);ctx.fillStyle="#a87930";ctx.font=`600 10px ${FONT}`;ctx.fillText("O N E - C L I C K",512,1509);ctx.fillStyle="#718078";ctx.font=`500 11px ${FONT}`;ctx.fillText("yjchufa.com · 不用查攻略，直接出发",512,1526);ctx.textAlign="left";
    const blob=await new Promise<Blob>((resolve,reject)=>canvas.toBlob(value=>value?resolve(value):reject(Object.assign(new Error("海报生成失败"),{code:"POSTER_EXPORT_INCOMPLETE"})),"image/jpeg",.92));return addJpegComment(blob,JSON.stringify({notice:"AI-generated visual illustration",model:spec.model,template:spec.version,generatedAt:new Date().toISOString()}));}));
}
export async function renderTravelPosters(spec: TravelPosterSpec) {
  await document.fonts?.ready;
  if("width" in spec)return renderTimelinePosterV2(spec);
