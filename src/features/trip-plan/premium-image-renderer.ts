...
async function renderTimelinePosterV2(spec:Extract<TravelPosterSpec,{version:"shaoxing_timeline_v2"}>){
  return Promise.all(spec.pages.map(async(page,pageIndex)=>{const canvas=document.createElement("canvas");canvas.width=1024;canvas.height=1536;const ctx=canvas.getContext("2d");if(!ctx)throw new Error("当前浏览器无法生成海报");ctx.fillStyle="#fbf8ef";ctx.fillRect(0,0,1024,1536);
    ...
    rounded(ctx,24,1306,976,154,18,"#eef3ec");ctx.fillStyle="#245b46";ctx.font=`800 18px ${FONT}`;ctx.fillText("出发前看一眼",44,1338);ctx.fillStyle="#4f5e55";ctx.font=`500 14px ${FONT}`;const tips=page.tips.slice(0,6);tips.forEach((tip,i)=>{const col=i%3,row=Math.floor(i/3);ctx.fillText(`· ${tip.slice(0,28)}`,44+col*318,1370+row*31);});ctx.fillStyle="#7d877f";ctx.font=`500 11px ${FONT}`;ctx.fillText("景点图片为 AI 视觉示意，请以实际现场为准。",44,1445);
