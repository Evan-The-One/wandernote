...
async function renderTimelinePosterV2(spec:Extract<TravelPosterSpec,{version:"shaoxing_timeline_v2"}>){
  return Promise.all(spec.pages.map(async(page,pageIndex)=>{const canvas=document.createElement("canvas");canvas.width=1024;canvas.height=1536;const ctx=canvas.getContext("2d");if(!ctx)throw new Error("当前浏览器无法生成海报");ctx.fillStyle="#fbf8ef";ctx.fillRect(0,0,1024,1536);
    ...
    const columns=page.days.length;const gap=18,left=24,cardWidth=columns===2?(1024-left*2-gap)/2:976;const top=154,bottom=1288,cardHeight=bottom-top;
    ...
    rounded(ctx,24,1306,976,154,18,"#eef3ec");ctx.fillStyle="#245b46";ctx.font=`800 18px ${FONT}`;ctx.fillText("出发前看一眼",44,1338);ctx.fillStyle="#4f5e55";ctx.font=`500 13px ${FONT}`;const advice=spec.preTripAdvice&&pageIndex===spec.pages.length-1?[spec.preTripAdvice.transport,spec.preTripAdvice.accommodation,spec.preTripAdvice.clothing,spec.preTripAdvice.photoSpots,spec.preTripAdvice.food,spec.preTripAdvice.timing]:page.tips.slice(0,3);advice.forEach((tip,i)=>{const col=i%3,row=Math.floor(i/3);ctx.fillText(`· ${tip.slice(0,36)}`,44+col*318,1370+row*31);});ctx.fillStyle="#7d877f";ctx.font=`500 11px ${FONT}`;ctx.fillText("景点图片为 AI 视觉示意，请以实际现场为准。",44,1445);
    ...
    return new Promise<Blob>((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error("海报生成失败")),"image/jpeg",.93));}));
}
