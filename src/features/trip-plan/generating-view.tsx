...
const stylePlanning: Record<TripInput["travelStyle"], string> = {
  fast_paced:
    "你选择了特种兵，我们会早出晚归、多逛多打卡，同时保留交通和吃饭时间。",
...
export function GeneratingView() {
  ...
  useEffect(() => {
    ...
    const rawInput =
      window.localStorage.getItem("yijianchufa:trip-input") ||
      window.localStorage.getItem("wandernote:demo-input");
    let input: ReturnType<typeof tripInputSchema.safeParse> | null = null;
    try {
      const migrated = rawInput ? migrateTripInput(JSON.parse(rawInput)) : null;
      input = migrated ? tripInputSchema.safeParse(migrated) : null;
    } catch {
      input = null;
    }
    if (!input?.success) {
      queueMicrotask(() =>
        setError("没有找到有效的旅行需求，请返回重新填写。"),
      );
      return;
    }
  ...
  if (error)
    return (
      <main className="page-shell flex min-h-[75vh] items-center justify-center py-12">
        <div className="card w-full max-w-lg rounded-3xl p-8 text-center">
          <BrandMark align="center" size="compact" />
          <div
            className="mx-auto mt-6 grid h-14 w-14 place-items-center rounded-full bg-[#fbe9e5] text-[#a34838]"
            aria-hidden="true"
          >
            <svg
              viewBox="0 0 24 24"
          ...
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            ...
            <button
              onClick={retry}
              className="rounded-full bg-[#245b46] px-5 py-3 font-semibold text-white"
            >
              使用原条件重新生成
            </button>
          </div>
  ...
  const styleLabel = travelStyles.find(
    (item) => item.value === summary?.travelStyle,
  )?.label;
  const priorityLabels =
    summary?.priorities
      .map(
        (value) => priorityOptions.find((item) => item.value === value)?.label,
      )
      .filter(Boolean) || [];
  return (
    <main className="page-shell flex min-h-[75vh] items-center justify-center py-12">
      <div className="w-full max-w-xl text-center">
        <BrandMark align="center" size="compact" />
        <div className="mx-auto mt-8 h-16 w-40" aria-hidden="true">
          <svg
            viewBox="0 0 160 64"
            fill="none"
            className="h-full w-full text-[#9ebdab]"
          >
            <path
              d="M8 45c25-34 46 13 72-14s44 17 72-13"
              stroke="currentColor"
              strokeWidth="2"
              strokeDasharray="4 6"
              strokeLinecap="round"
            />
            <circle cx="8" cy="45" r="5" className="travel-dot travel-dot-1" />
            <circle cx="80" cy="31" r="5" className="travel-dot travel-dot-2" />
            <circle cx="152" cy="18" r="5" className="travel-dot travel-dot-3" />
          </svg>
        </div>
        <h1 className="mt-5 text-3xl font-bold">正在为你安排这几天</h1>
        ...
        {summary && (
          <div className="mt-6 rounded-2xl bg-white p-4 text-left shadow-sm">
            <div className="flex flex-wrap gap-2 text-sm font-bold">
              <span>{summary.destination.city}</span>
              ...
              {priorityLabels.length > 0 && (
                <>
                  <span className="text-[#a0a6a2]">·</span>
                  <span>{priorityLabels.join("、")}</span>
                </>
              )}
            </div>
            <p className="mt-3 text-sm leading-6 text-[#65706a]">
              {stylePlanning[summary.travelStyle]}
            </p>
          </div>
        )}
        <PlanningProgress steps={steps} intervalMs={8000} />
        <p className="mt-6 text-xs leading-5 text-[#828a85]">
          页面提示用于展示规划过程，实际完成时间以生成结果为准。
        </p>
      </div>
    </main>
  );
