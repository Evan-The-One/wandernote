...
export function EmailLoginForm({ returnTo = "/" }: { returnTo?: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-7">
      ...
      <button type="submit" disabled={!email || sending} className="mt-4 min-h-12 w-full rounded-full bg-[#245b46] px-5 py-3 font-bold text-white disabled:opacity-45">{sending ? "正在发送…" : "发送登录链接"}</button>
