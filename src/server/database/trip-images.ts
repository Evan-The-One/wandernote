...
import { genericVisualCategory, genericVisualDataUrl, type GenericVisualCategory } from "@/server/images/generic-visuals";
import { posterPointCost } from "@/config/commerce";

export const PREMIUM_IMAGE_TEMPLATE_VERSION = "classic_timeline_v1";
export const TRAVEL_POSTER_VERSION = "realistic_generic_visuals_v4";
...
const VISUAL_STYLE_VERSION = "realistic_editorial_v1";
...
function visualKey(destination:string,name:string,category:PosterCategory,model:string,generic?:GenericVisualCategory){return createHash("sha1").update(["CN",isGenericCategory(category)?"GLOBAL":destination,normalizePlaceName(assetName(name,category,generic)),category,VISUAL_STYLE_VERSION,model,POSTER_PROMPT_VERSION].join("|")).digest("hex");}
...
export async function createTravelPosterTask(args: { tripId: string; visitorId: string; userId: string; aspectRatio: TripImageAspectRatio; idempotencyHash: string; lifetimeLimit: number }) {
  ...
  try {
    ...
    for(const day of days){const missing:typeof day.activities=[];for(const activity of day.activities){const generic=isGenericCategory(activity.category)?genericVisualCategory(activity.name,activity.category,activity.note):undefined;const key=visualKey(plan.destination.city,activity.name,activity.category,model,generic),cached=cache.get(key);if(cached){reusedCount++;results.set(`${day.dayNumber}:${activity.time}:${activity.name}`,{...cached,reused:true});const destination=generic?"GLOBAL":plan.destination.city,name=generic?`__generic_${generic}`:activity.name;await db.update(placeVisualAssets).set({reuseCount:sql`${placeVisualAssets.reuseCount}+1`,updatedAt:new Date()}).where(and(eq(placeVisualAssets.destination,destination),eq(placeVisualAssets.normalizedPlaceName,normalizePlaceName(name)),eq(placeVisualAssets.visualStyleVersion,VISUAL_STYLE_VERSION)));}else if(generic){const dataUrl=await genericVisualDataUrl(generic);const asset={dataUrl,category:activity.category,altText:`${activity.name}真实场景视觉`,reused:true};results.set(`${day.dayNumber}:${activity.time}:${activity.name}`,asset);cache.set(key,asset);genericVisuals++;await saveVisualAsset({destination:"GLOBAL",name:`__generic_${generic}`,category:activity.category,model,dataUrl,cost:0,isGeneric:true});}else missing.push(activity);}if(!missing.length)continue;const generated=await provider.generatePosterBackground({prompt:visualPrompt(plan.destination.city,missing),size:"1024x1024",quality:"low"});estimatedCostUsd+=generated.estimatedCostUsd;const crops=await cropContactSheet(generated.dataUrl,missing.length);for(let index=0;index<missing.length;index++){const activity=missing[index]!,key=visualKey(plan.destination.city,activity.name,activity.category,model),asset={dataUrl:crops[index]!,category:activity.category,altText:`${activity.name} AI视觉示意`,reused:false};cache.set(key,asset);results.set(`${day.dayNumber}:${activity.time}:${activity.name}`,asset);generatedVisuals++;await saveVisualAsset({destination:plan.destination.city,name:activity.name,category:activity.category,model,dataUrl:crops[index]!,cost:generated.estimatedCostUsd/missing.length});}}
    const hydratedDays=days.map(day=>({...day,activities:day.activities.map(activity=>{const generic=isGenericCategory(activity.category)?genericVisualCategory(activity.name,activity.category,activity.note):undefined;const key=visualKey(plan.destination.city,activity.name,activity.category,model,generic);const result=results.get(`${day.dayNumber}:${activity.time}:${activity.name}`);if(!result)throw Object.assign(new Error("活动图片缺失"),{code:"POSTER_VISUAL_MISSING"});return {...activity,visualAsset:{id:randomUUID(),cacheKey:key,...result}};})}));
