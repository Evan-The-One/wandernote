...
export const TRAVEL_POSTER_VERSION = "oneclick_travel_cjk_v8";
...
function categoryFor(visualCategory:GenericVisualCategory|null,type:string):PosterCategory {
  if(visualCategory?.includes("hotel")||visualCategory==="luggage_pickup")return "hotel";
  if(visualCategory?.includes("lunch")||visualCategory?.includes("dinner")||visualCategory?.includes("breakfast")||visualCategory==="cafe_generic")return "food";
  if(visualCategory&&["self_drive_departure","return_trip","train_travel","airplane_travel","transit_departure","city_walking"].includes(visualCategory))return "transport";
  if(visualCategory==="rest_generic")return "rest";
  if(visualCategory&&["walking_street","city_stroll","historic_street"].includes(visualCategory))return "shopping";
  if(type==="shopping")return "shopping";if(type==="entertainment")return "entertainment";return "attraction";
}
...
async function saveVisualAsset(args:{destination:string;name:string;category:PosterCategory;model:string;dataUrl:string;cost:number;isGeneric?:boolean}){
  ...
  const sharp=await loadSharp(),input=Buffer.from(args.dataUrl.split(",")[1]||"","base64"),image=sharp(input),metadata=await image.metadata(),stats=await image.stats();
  ...
  const name=args.isGeneric?args.name:assetName(args.name,args.category);
...
export async function createTravelPosterTask(args: { tripId: string; visitorId: string; userId: string; aspectRatio: TripImageAspectRatio; idempotencyHash: string; lifetimeLimit: number }) {
  ...
  try {
    ...
    const results=new Map<string,SelectedVisual>(),usedContentHashes=new Set<string>(),usedVisualKeys=new Set<string>();
      ...
      for(const activity of day.activities){
        ...
        if(activity.visualCategory){
          ...
          for(const candidate of candidates){
            const dataUrl=await genericVisualDataUrl(candidate.key),contentHash=createHash("sha256").update(dataUrl).digest("hex"),cacheKey=visualKey(plan.destination.city,activity.name,activity.category,model,visualCategory,candidate.key);
            if(usedContentHashes.has(contentHash)||usedVisualKeys.has(cacheKey))continue;
            selected={dataUrl,category:activity.category,altText:`${activity.name}真实场景视觉`,reused:true,cacheKey,contentHash};
            await saveVisualAsset({destination:"GLOBAL",name:`__generic_${candidate.key}`,category:activity.category,model,dataUrl,cost:0,isGeneric:true});
            genericVisuals++;break;
          }
          // Repetition is allowed only after every legal candidate in the exact semantic category is exhausted.
          if(!selected&&candidates.length){
            const candidate=candidates.find(item=>!usedVisualKeys.has(visualKey(plan.destination.city,activity.name,activity.category,model,visualCategory,item.key)))||candidates[0]!;
            const dataUrl=await genericVisualDataUrl(candidate.key),contentHash=createHash("sha256").update(dataUrl).digest("hex"),cacheKey=visualKey(plan.destination.city,activity.name,activity.category,model,visualCategory,candidate.key);
            selected={dataUrl,category:activity.category,altText:`${activity.name}真实场景视觉`,reused:true,cacheKey,contentHash};
          }
          if(!selected)throw Object.assign(new Error("没有合法的同类别活动视觉"),{code:"POSTER_VISUAL_FALLBACK_INVALID"});
          usedContentHashes.add(selected.contentHash);usedVisualKeys.add(selected.cacheKey);results.set(resultKey,selected);reusedCount++;continue;
        }
        const key=visualKey(plan.destination.city,activity.name,activity.category,model),cached=cache.get(key);
        if(cached){
          const contentHash=createHash("sha256").update(cached.dataUrl).digest("hex");
          if(!usedContentHashes.has(contentHash)){const selected={...cached,reused:true,cacheKey:key,contentHash};usedContentHashes.add(contentHash);usedVisualKeys.add(key);results.set(resultKey,selected);reusedCount++;continue;}
        }
