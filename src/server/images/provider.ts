...
export interface ImageGenerationProvider {
  ...
  generatePosterBackground(input: { prompt: string; size: "1024x1536" }): Promise<PosterImageResult>;
}
...
export function createOpenAIImageProvider(): ImageGenerationProvider {
  ...
  return {
    ...
    async generatePosterBackground({ prompt, size }) {
      ...
      try {
        const response = await fetch("https://api.openai.com/v1/images/generations", {
          ...
          body: JSON.stringify({ model, prompt: prompt.slice(0, 7000), size, quality, output_format: "webp", output_compression: 72, moderation: "auto", n: 1 }),
        ...
        return { dataUrl: `data:image/webp;base64,${payload.data[0].b64_json}`, model, estimatedCostUsd: PRICE_BY_QUALITY[quality] };
      } catch (error) {
