...
export const POSTER_RENDER_VERSION="sharp_svg_brand_a_v2";
  ...
  for(const[pageIndex,page]of spec.pages.entries()){
    ...
    for(const[column,day]of page.days.entries()){
      ...
      for(let i=0;i<count;i++){
        ...
        svg+=`<circle cx="${timelineX}" cy="${rowY+25}" r="7" fill="${i===0?"#f2c14e":"#b9db8a"}" stroke="#1f6b4f" stroke-width="2"/>`+text(textX,rowY+19,activity.time,columns===2?18:20,800,"#1f6b4f");
