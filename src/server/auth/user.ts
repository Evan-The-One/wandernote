...
async function ensureUserAuthTables(){if(initialized)return;const db=getDatabase();await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS users (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),email text NOT NULL UNIQUE,verified_at timestamptz,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),last_login_at timestamptz)`));await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS user_sessions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,token_hash text NOT NULL UNIQUE,expires_at timestamptz NOT NULL,created_at timestamptz NOT NULL DEFAULT now())`));await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS email_login_tokens (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),email text NOT NULL,token_hash text NOT NULL UNIQUE,expires_at timestamptz NOT NULL,used_at timestamptz,created_at timestamptz NOT NULL DEFAULT now())`));await db.execute(sql.raw(`ALTER TABLE trips ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES users(id) ON DELETE SET NULL`));await db.execute(sql.raw(`ALTER TABLE entitlement_ledger ADD COLUMN IF NOT EXISTS principal_type text`));await db.execute(sql.raw(`ALTER TABLE entitlement_ledger ADD COLUMN IF NOT EXISTS principal_id uuid`));initialized=true;}
