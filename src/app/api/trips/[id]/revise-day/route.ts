...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json({ plan: updatedPlan, version: body.data.version + 1, changeSummary: result.changeSummary });
  } catch (error) {
