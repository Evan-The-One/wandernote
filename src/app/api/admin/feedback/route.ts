...
export async function GET(request: Request) {
  ...
  const allRows = await db.select({ id: feedback.id, tripId: feedback.tripId, posterTaskId:feedback.posterTaskId,feedbackType:feedback.feedbackType,status:feedback.status,rating: feedback.rating, tags: feedback.issueTagsJson, comment: feedback.comment, createdAt: feedback.createdAt, tripCreatedAt: trips.createdAt, input: trips.inputJson }).from(feedback).innerJoin(trips, eq(feedback.tripId, trips.id)).where(where).orderBy(desc(feedback.createdAt));
...
const updateSchema=z.object({id:z.string().uuid(),status:z.enum(["new","reviewing","resolved"])});
export async function PATCH(request:Request){try{assertTrustedMutation(request);if(!await hasAdminAccess())throw new HttpError(401,"未授权","ADMIN_REQUIRED");const parsed=updateSchema.safeParse(await readJsonBody(request));if(!parsed.success)throw new HttpError(400,"参数无效","INVALID_FEEDBACK_STATUS");await getDatabase().update(feedback).set({status:parsed.data.status,updatedAt:new Date()}).where(eq(feedback.id,parsed.data.id));return Response.json({ok:true});}catch(error){return apiError(error)}}
