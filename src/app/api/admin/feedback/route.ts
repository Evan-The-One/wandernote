...
export async function GET(request: Request) {
  ...
  const parsed = allRows.map((row) => { const match = row.comment?.match(/^第(\d+)天反馈$/); return { ...row, kind: match ? "day" : "overall", dayNumber: match ? Number(match[1]) : null, destination: row.input.destination.city, style: row.input.travelStyle, interests: row.input.priorities, companion: row.input.companionType, hasCustomTime: Boolean(row.input.preferredWakeTime || row.input.preferredDepartureTime), comment: match ? null : row.comment }; }).filter((row) => (!kind || kind === "all" || row.kind === kind) && (!style || row.style === style) && (!companion || row.companion === companion) && (!interest || row.interests.includes(interest as never)) && (!tag || row.tags.includes(tag)));
