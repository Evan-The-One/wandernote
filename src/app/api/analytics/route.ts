...
const eventSchema=z.object({eventName:z.enum(["page_view","page_duration","generate_clicked","share_clicked","day_image_saved","premium_image_saved","premium_image_shared","feedback_submitted","retry_clicked","network_error","user_aborted","poster_example_open_top","poster_example_open_bottom","poster_generate_click_top","poster_generate_click_bottom","poster_login_prompt_shown","poster_generation_confirmed","poster_generation_started","poster_generation_succeeded","poster_generation_failed","poster_save_current","poster_save_all","poster_share"]),tripId:z.string().uuid().nullable().optional(),pageName:z.enum(["home","generating","trip","other"]).nullable().optional(),durationMs:z.number().int().min(0).max(30*60*1000).nullable().optional(),status:z.string().max(30).nullable().optional(),metadata:z.record(z.string(),z.union([z.string().max(50),z.number(),z.boolean(),z.null()])).refine(value=>Object.keys(value).length<=12).optional()});
