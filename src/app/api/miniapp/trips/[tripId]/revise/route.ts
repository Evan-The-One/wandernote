...
export async function POST(request:Request,{params}:{params:Promise<{tripId:string}>}){
  ...
  try{
    const user=await currentMiniappUser(request),body=schema.parse(await readMiniappJson(request)),{tripId}=await params;const requestId=request.headers.get("idempotency-key")?.trim();if(!requestId||requestId.length<16||requestId.length>128)throw new HttpError(400,"请重新提交修改","INVALID_IDEMPOTENCY_KEY");
