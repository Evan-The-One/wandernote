...
const schema=z.object({version:z.number().int().positive(),dayNumber:z.number().int().min(1).max(7),instruction:z.string().trim().min(2).max(500)});
...
export async function POST(request:Request,{params}:{params:Promise<{tripId:string}>}){
  ...
  try{
    ...
    const visitorId=await miniappVisitor(user.id),access=await resolveGenerationAccess(user.id);if(access.enforceDailyWholeDayRevisionLimit)await assertRevisionModeLimit(visitorId,"full_day");const guard=await assertAiRequestAllowed(request,visitorId,"day_revision");jobId=await createRevisionJob(visitorId,tripId);
    const result=await reviseDay({schemaVersion:"0.2",originalInput:row.inputJson,strategy:plan.strategy,budget:plan.budget,targetDayNumber:currentDay.dayNumber,currentDay,previousDay:adjacent(plan.days[index-1]),nextDay:adjacent(plan.days[index+1]),otherDaysCostTotal:null,instruction:body.instruction,mode:"full_day",selectedActivityIds:[]},usage=>recordAiUsage(visitorId,tripId,jobId!,usage,true,access.mode),!guard.softBudgetReached);
